...
\begin{abstract}
Product formulas are the most widely used primitive for the quantum simulation of
time evolution, and the order in which a Hamiltonian's terms are applied within a
first-order (Lie--Trotter) step is a free parameter that leaves the gate count
unchanged. Despite extensive study, however, term-ordering heuristics deliver only
marginal and seemingly irreducible accuracy gains, and the reason has remained
unclear. We tackle this problem by introducing \emph{commutator-graph scheduling}
(CGS), a framework that recasts the leading first-order error as an exact,
matrix-free operator---the \emph{commutator error form}
$E_\pi=\sum_{a<b}[h_{\pi(a)},h_{\pi(b)}]$---built as a signed sum of Pauli strings
over the edges of the Hamiltonian's commutator graph and validated against dense
matrix exponentials to $10^{-9}$. From this representation we analytically prove
that term ordering is impotent: reordering only re-signs the commutators in
$E_\pi$, leaving its norm exactly invariant whenever the edge commutators are
distinct, so that no fixed ordering escapes the $O(t^2/r)$ first-order rate. We
then show that the genuinely free parameter is not the ordering but the per-step
\emph{schedule}: a step-alternating \emph{antithetic} schedule cancels $E_\pi$
exactly and converges at $O(t^3/r^2)$ while applying the identical $L\cdot r$
rotations. Across $\Ninstances$ structured Hamiltonian instances
(transverse-field Ising, Heisenberg, and molecular-like families, $4$--$8$ qubits)
evaluated against exact statevector references, CGS increases the empirical
convergence rate from $\SlopeFirst$ to $\SlopeAnti$ and reaches fidelity $0.99$
with $\GateSpeedup\times$ fewer rotations than the best first-order ordering, while
attaining $0.999$ where first-order Trotter does not. Finally, an interpretable
learned scheduler over ten commutator-graph features selects the gate-optimal
schedule per instance with $\LearnedAcc\%$ leave-one-out accuracy, recovering
near-oracle efficiency. By improving the accuracy of first-order Trotterization
while requiring no additional gates, qubits, or circuit depth, commutator-graph
scheduling offers a zero-overhead, drop-in upgrade for quantum simulation on
near-term hardware.
\end{abstract}
...
computers~\cite{feynman1982simulating,lloyd1996universal}. The workhorse primitive
...
accuracy it can buy~\cite{tranter2019ordering,poulin2015trotter,hastings2015improving}.
...
impotent has not, to our knowledge, been explained.

We tackle this problem, and turn it into an asset, by introducing
...
symmetrization of Suzuki~\cite{suzuki1991general,hatano2005finding}; its reading as a
zero-overhead \emph{scheduling} choice---one that the impotence theorem shows a
single fixed ordering can never realize---is, to our knowledge, new, and it changes
the practical recommendation for first-order simulation.

We make the following contributions.
\begin{itemize}[leftmargin=1.2em,itemsep=2pt,topsep=2pt]
    \item \textbf{An exact error certificate.} We introduce the commutator error
      form [Eq.~\eqref{eq:errorform}], a matrix-free representation of the leading
      Trotter error as a signed Pauli-string sum over the commutator graph,
      validated against the dense pair-commutator sum to $10^{-9}$
      (Definition~\ref{def:errorform}).
    \item \textbf{An ordering-impotence theorem.} We analytically prove that a
      single ordering cannot reduce the leading-error norm below an instance floor
      that is exactly ordering invariant on collision-free families
      (Theorem~\ref{thm:impotence}), explaining the empirical ceiling that ordering
      heuristics encounter.
    \item \textbf{A free second-order schedule.} We prove that the antithetic
      schedule cancels the leading error and converges at second order at the
      identical gate budget (Theorem~\ref{thm:antithetic}), and combine it with a
      commuting-clique compression of the commutator graph
      (Proposition~\ref{prop:colour}).
    \item \textbf{A matched-cost benchmark.} We benchmark CGS against exact
      statevector references on $\Ninstances$ structured Hamiltonian instances,
      reporting rotations to a target fidelity and the empirical convergence rate;
      CGS reaches $0.99$ with $\GateSpeedup\times$ fewer rotations and is the only
      family of schedules to reach $0.999$ within the swept budget.
    \item \textbf{An interpretable scheduler.} We add a learned scheduler over ten
      commutator-graph features that selects the gate-optimal schedule per instance
      with $\LearnedAcc\%$ leave-one-out accuracy and recovers near-oracle efficiency
      (Proposition~\ref{prop:invariance}).
\end{itemize}

We state the boundaries of the study plainly. CGS is evaluated by classical
statevector simulation on small systems ($n\le 8$ qubits) where an exact reference
is tractable, on synthetic and structured Hamiltonians, under a rotation-count cost
model; the gains are asymptotic in the step size, and we report the crossover budget
honestly. The antithetic schedule is not a new product-formula primitive---the
...
learned scheduler that selects it, is, to our knowledge, new.
...
no number is reported until two integrity gates pass: the fast Pauli-rotation kernel
and dense \texttt{expm} agree to $10^{-9}$, and the matrix-free error form agrees
with the dense pair-commutator sum to $10^{-9}$.
...
the exact reference $e^{-\ii Ht}\ket{\psi_0}$ produced by dense \texttt{expm}
(cross-checked against Krylov \texttt{expm\_multiply}), with probe state
$\ket{+}^{\otimes n}$.
...
exact leading-error spectral norm, the coefficient ordering sits within a factor
$\ImpCoeff$ of the per-instance minimum on average, a median random ordering within
$\ImpMedian$, and the worst single instance across all $\Ninstances$ instances reaches
only $\ImpMax$; the matrix-free Hilbert--Schmidt surrogate moves even less, within
$\ImpHS$. We emphasize that this is not a statement about a particular heuristic but
about \emph{every} ordering, and that it is the precise, provable form of the
``ordering is within noise'' folklore. A single ordering simply cannot escape the
...
$\approx2$ to $\approx4$. We emphasize that the antithetic schedule is not only more
accurate than every first-order ordering, it simultaneously preserves the
\emph{identical} $L\cdot r$ rotation count at each $r$; only its per-step direction
differs.
...
schedule and $\approx4$ for both folds. We note that the symmetric fold reaches the
highest fidelity but spends $\approx2\times$ rotations per step to do so, whereas the
...
to the cheap first-order schedule and hard instances to a fold. We emphasize that the
scheduler is interpretable and listing-order invariant by construction
...
higher-order product formulas are classical~\cite{suzuki1991general,hatano2005finding},
and randomized compilation---random permutation per step and qDRIFT---improves error
scaling by averaging rather than
cancellation~\cite{childs2019faster,campbell2019random}; the antithetic schedule is the
...
the randomized baselines. The tight commutator-scaling theory of Trotter
...
We report the boundaries of the study explicitly. (i)~The second-order advantage is a
...
below near the intermediate budget); we report the crossover rather than hide it.
(ii)~The exact statevector reference requires forming $e^{-\ii Ht}$, limiting us to
$n\le8$ qubits, so the effect at scale---where a tensor-network or Krylov reference is
needed---is not measured, although the convergence-rate argument is size independent.
...
first-order Trotter compilation can adopt unchanged on near-term hardware. Natural next
steps are larger systems with scalable references, real mapped molecular Hamiltonians,
higher-order folded schedules, and a graph neural policy trained directly on a
differentiable matched-cost objective. In short, by recasting the leading Trotter error
...
interests, and thanks the maintainers of the open-source scientific Python
...
This study uses no external datasets. All Hamiltonians are generated synthetically from
fixed random seeds by the accompanying code, and all numerical values, tables, and
figures are produced by that code and stored in \texttt{results/summary.json}. The code
that implements CGS and regenerates every figure, table, and reported value accompanies
this manuscript as an installable package (\texttt{pip install .}) and will be released
...
configuration (\texttt{configs/full.yaml}) uses sizes $n\in\{4,5,6,7,8\}$, eight seeded
instances per (family, size), evolution time $t=\EvolTime$, a reference budget of
$r=\RefSteps$ steps, and a step grid $\{1,2,4,6,8,12,16,24\}$ for the convergence and
frontier curves, giving $\Ninstances$ instances in total. For each instance and schedule
we evaluate the product formula, score fidelity and infidelity against the exact
reference, and record the realized rotation count; from the (rotation count, fidelity)
curves we read the rotations to each target fidelity and fit the convergence rate.
Ordering impotence is measured by sampling $48$ orderings per instance and forming the
exact leading-error spectral norm. Per-schedule means carry 95\% confidence intervals
...
Sec.~\ref{sec:router_thy}, trained leave-one-instance-out.
...
two integrity flags that must pass before any fidelity is produced:
...
Python~3.13, macOS arm64), the experiment completed in $\approx\Runtime$\,s wall-clock
with $\approx\PeakMem$\,MB peak memory, both flags \texttt{true}. The macros, tables,
and figures in this manuscript are generated directly from \texttt{summary.json}, so no
number is entered by hand. The full pipeline is reproduced from the accompanying code by
\begin{verbatim}
pip install .
make test
