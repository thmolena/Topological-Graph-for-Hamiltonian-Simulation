...
> pip-PyTorch can each ship an OpenMP runtime; the setting averts the duplicate-libomp abort
...
|---|---|---|
| `results/summary.json` | `scripts/run.py` | Single source of truth: per-ordering fidelity (mean, std, 95% CI), infidelity, Z₀ observable error, anticommuting-adjacency count, the fidelity-vs-gate-budget frontier, the per-family breakdown, and the `pauli_algebra_verified` integrity flag. |
| `results/main_results.tex` | `scripts/make_tables.py` | LaTeX table of the headline per-ordering metrics at the matched gate proxy. |
...
before any fidelity number was produced. It is the single source of truth for every table,
figure, and macro.
